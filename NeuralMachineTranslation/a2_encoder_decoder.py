...
class Encoder(EncoderBase):
    ...
    def get_all_hidden_states(self, x, F_lens, h_pad):
        ...
        # call padded sequence
        packed_sequence = torch.nn.utils.rnn.pack_padded_sequence(x, F_lens,  enforce_sorted = False)
        # rrn_hidden - (2n, N, H) but want (S, N, 2H)
        rnn_output, rnn_hidden = self.rnn(packed_sequence)
        return torch.nn.utils.rnn.pad_packed_sequence(rnn_output, padding_value = h_pad)[0] # dont want lens again
...
class DecoderWithAttention(DecoderWithoutAttention):
    ...
    def init_submodules(self):
        ...
        # cell_type will be one of: ['lstm', 'gru', 'rnn']
        if self.cell_type == 'lstm':
            # + 1 due to c is vector of n x 2h
    ...
    def get_first_hidden_state(self, h, F_lens):
        ...
        return torch.zeros_like(h[0]) # only dim N x 2h

    def get_current_rnn_input(self, E_tm1, htilde_tm1, h, F_lens):
        # update to account for attention. Use attend() for c_t
        E_t = self.embedding(E_tm1)
    ...
    def attend(self, htilde_t, h, F_lens):
        ...
        return torch.einsum('snh,sn->nh', h, attention_weights)
    ...
    def get_energy_scores(self, htilde_t, h):
        ...
        s = h.shape[0]
        if self.cell_type == 'lstm':
            htilde_t = htilde_t[0] #just need h, not c
        htilde_new = htilde_t.repeat(s, 1, 1)
        return torch.cosine_similarity(htilde_new, h, dim=2)
...
class EncoderDecoder(EncoderDecoderBase):
    ...
    def init_submodules(self, encoder_class, decoder_class):
        ...
        self.encoder = encoder_class(self.source_vocab_size, self.source_pad_id, self.word_embedding_size,
            self.encoder_num_hidden_layers, self.encoder_hidden_size, self.encoder_dropout, self.cell_type)
